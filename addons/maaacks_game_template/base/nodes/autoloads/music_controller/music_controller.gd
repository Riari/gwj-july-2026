...
func _set_sub_audio_volume_linear(sub_volume_linear: float) -> void:
	AudioServer.set_bus_volume_linear(blend_audio_bus_idx, sub_volume_linear)

func fade_in(duration: float = 0.0) -> Tween:
	if is_zero_approx(duration): return
	music_stream_player.bus = blend_audio_bus
	AudioServer.set_bus_volume_linear(blend_audio_bus_idx, 0.0)
	var tween = create_tween()
	tween.tween_method(_set_sub_audio_volume_linear, 0.0, 1.0, duration)
	return tween

func blend_to(target_volume_linear: float, duration : float = 0.0) -> Tween:
	if not is_zero_approx(duration):
		var tween = create_tween()
		tween.tween_property(music_stream_player, "volume_linear", target_volume_linear, duration)
		return tween
	music_stream_player.volume_linear = target_volume_linear
	return
